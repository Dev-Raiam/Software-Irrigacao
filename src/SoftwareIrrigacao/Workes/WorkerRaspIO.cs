        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var responseEscritaDigital = await _controller.WriteDigitalAsync(
                        10,
                        false,
                        stoppingToken
                    );
                    var responseEscritaAnalogica = await _controller.WriteAnalogAsync(
                        10,
                        1024,
                        stoppingToken
                    );
                    ...
                    _logger.LogInformation(
                        "Comando de Excrita enviado com sucesso!!! {response}",
                        responseDigital
                    );
                    _logger.LogInformation(
                        "Comando de Excrita enviado com sucesso!!! {response}",
                    ...
                    _logger.LogInformation(
                        "Comando de Excrita enviado com sucesso!!! {response}",
                        responseEscritaDigital
                    );
                    _logger.LogInformation(
                        "Comando de Excrita enviado com sucesso!!! {response}",
                        responseEscritaAnalogica
                    );
                }
                catch (Exception ex)
                {
